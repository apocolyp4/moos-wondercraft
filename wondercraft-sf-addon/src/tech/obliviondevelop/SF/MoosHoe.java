...
public class MoosHoe  extends SimpleSlimefunItem<ToolUseHandler> implements NotPlaceable, DamageableItem
{
    ...
    private void breakBlocks(BlockBreakEvent e, Player p, ItemStack item, Block b, List<Block> blocks, List<ItemStack> drops) {
        ...
        ExplosiveToolBreakBlocksEventMooHoe event = new ExplosiveToolBreakBlocksEventMooHoe(p, b, blocksToDestroy, item, this);
    ...
    private List<Block> findBlocks(@Nonnull Block b) {
        ...
        for (int x =  -block_radius; x <= block_radius; x++) {
            for (int y = 0; y <= 0; y++) {
                for (int z =  -block_radius; x <= block_radius; z++) {
