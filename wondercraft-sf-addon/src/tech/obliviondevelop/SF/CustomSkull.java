...
public class CustomSkull {
	...
	{
		if (!SlimefunPlugin.getCfg().contains("skulls.uuids." + texture))
		{
			SlimefunPlugin.getCfg().setValue("skulls.uuids." + texture, UUID.randomUUID().toString());
			SlimefunPlugin.getCfg().save();
		}
		Object profile = profile_constructor.newInstance(UUID.fromString(SlimefunPlugin.getCfg().getString("skulls.uuids." + texture)), "CSCoreLib");
